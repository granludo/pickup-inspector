...
int volumeMeasure(){
  ...
  while(!flag_end){
    ...
    if(flag_switch){
      delay(1000); //Para que el circuito a la salida no tenga un +Vdd y baje a 0V
      if(flag_print == 1){
  ...
  if(Serial.available() > 0){
    ...
    while(flag_init){
      ...
      if(MUX == 4){
        ...
        phi_m = atan(2*M_PI*f_m*C_trt*88000);
        phi_ph = atan(2*M_PI*f_ph*C_ph*88000);
        phi_test = atan(2*M_PI*f_test*C_trt*88000);
        phi_test_ph = atan(2*M_PI*f_test_ph*C_ph*88000);
        //Serial.print("phi_m = ");
